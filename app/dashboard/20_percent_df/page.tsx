...
export default function TwentyPercentDFLanding() {
    ...
    const isLoadingData = allTwentyPercentDF === undefined;

    // Calculate statistics per year
    const yearsWithStats = useMemo(() => {
        if (!fiscalYears || !allTwentyPercentDF) return [];

        return fiscalYears.map((fiscalYear) => {
            const yearFunds = allTwentyPercentDF.filter(fund => fund.year === fiscalYear.year);
            ...
            // Balance is total allocated - utilized for 20% DF context typically, or if there's a specific balance field
            // Checking schema: totalBudgetAllocated, obligatedBudget, totalBudgetUtilized
            // Usually Balance = Allocated - Utilized (or Obligated depending on accounting). 
            // In SEF it was explicitly 'balance'. usage in SEF page: `fund.balance`
            // In 20% DF schema: `totalBudgetAllocated`, `totalBudgetUtilized`, `obligatedBudget`.
            // Let's assume Balance is Allocated - Utilized for now unless schema has dedicated balance field.
            // Wait, schema (Step 13) doesn't have a 'balance' field on the root object!
            // It has totalBudgetAllocated, obligatedBudget, totalBudgetUtilized.
            // Let's calculate Balance dynamically.
            const totalBalance = yearFunds.reduce((sum, fund) => sum + ((fund.totalBudgetAllocated || 0) - (fund.totalBudgetUtilized || 0)), 0);
    ...
    }, [fiscalYears, allTwentyPercentDF]);
    ...
    const handleOpenYear = (year: number) => {
        router.push(`/dashboard/20_percent_df/${year}`);
    };
