// app/dashboard/project/[year]/[particularId]/components/form/ParticularField.tsx
...
export function ParticularField({ form, isEditMode }: ParticularFieldProps) {
  const allParticulars = useQuery(api.projectParticulars.list, {
    includeInactive: false,
  });
  const createParticular = useMutation(api.projectParticulars.create);
  ...
  const handleStartEdit = () => {
    setEditedParticular(form.getValues("particulars"));
  ...
  const handleSaveEdit = async () => {
    ...
    if (trimmed.length === 0 || !/^[\p{L}0-9_%\s,.\-@]+$/u.test(trimmed)) {
      toast.error("Invalid format", {
        description: "Code can only contain letters (including accents), numbers, underscores, percentage signs, spaces, commas, periods, hyphens, and @"
      });
      return;
    }
    ...
    if (!particularExists(trimmed)) {
      const shouldCreate = confirm(
        `Project particular "${trimmed}" doesn't exist yet.\n\nDo you want to create it now?`
      );
      ...
      try {
        ...
        await createParticular({
          code: trimmed,
          fullName: trimmed,
          description: `Auto-created from project edit: ${trimmed}`,
          category: "Custom",
        });

        toast.success("Project particular created!", {
          description: `"${trimmed}" has been added. You can edit details in Settings.`,
        });

        form.setValue("particulars", trimmed, { shouldValidate: true });
