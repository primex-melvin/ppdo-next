/**
 * Trust Fund Breakdown Activity Logger
 *
 * Centralized activity logging for Trust Fund Breakdown operations.
 * Tracks all changes with comprehensive metadata and change tracking.
 *
 * @module convex/lib/trustFundBreakdownActivityLogger
 */
...
/**
 * Configuration for Trust Fund Breakdown activity logging
 */
export interface TrustFundBreakdownActivityLogConfig {
  action:
    | "created"
    | "updated"
    | "deleted"
    ...
    | "exported"
    | "restored";
  breakdownId?: Id<"trustFundBreakdowns">;
  breakdown?: Doc<"trustFundBreakdowns"> | null;
  previousValues?: any;
  newValues?: any;
  batchId?: string;
  reason?: string;
...
/**
 * Log an activity for a Trust Fund Breakdown
 *
 ...
 * });
 */
export async function logTrustFundBreakdownActivity(
  ctx: MutationCtx,
  userId: Id<"users">,
  config: TrustFundBreakdownActivityLogConfig
): Promise<Id<"trustFundBreakdownActivities">> {
  const now = Date.now();
  ...
  let trustFundId: Id<"trustFunds"> | undefined;
  ...
  if (config.breakdown) {
    ...
    trustFundId = config.breakdown.trustFundId;
    ...
    trustFundId = config.previousValues.trustFundId;
    ...
    trustFundId = config.newValues.trustFundId;
  ...
  const activityId = await ctx.db.insert("trustFundBreakdownActivities", {
    // Action
