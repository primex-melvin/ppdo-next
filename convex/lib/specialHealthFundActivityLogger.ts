// convex/lib/specialHealthFundActivityLogger.ts
...
export interface SpecialHealthFundLogConfig {
    action: "created" | "updated" | "deleted" | "restored";
    specialHealthFundId?: Id<"specialHealthFunds">;
    specialHealthFund?: any;
    previousValues?: any;
    newValues?: any;
    reason?: string;
}
...
/**
 * Log special health fund activity with comprehensive tracking
 */
export async function logSpecialHealthFundActivity(
    ctx: MutationCtx,
    userId: Id<"users">,
    config: SpecialHealthFundLogConfig
): Promise<void> {
    try {
        ...
        // Snapshot Data
        const fundData = config.specialHealthFund || config.newValues || config.previousValues || {};
        ...
        await ctx.db.insert("specialHealthFundActivities", {
            action: config.action,
            specialHealthFundId: config.specialHealthFundId,
            projectTitle: fundData.projectTitle || "Unknown Special Health Fund",
        ...
        console.error("Failed to log special health fund activity:", error);
    }
}
