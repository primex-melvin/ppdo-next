...
/**
 * Create a new trust fund remark
 */
export const createRemark = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Verify trust fund exists
    const fund = await ctx.db.get(args.trustFundId);
    if (!fund) {
    ...
    const remarkId = await ctx.db.insert("trustFundRemarks", {
...
export const updateRemark = mutation({
  args: {
    remarkId: v.id("trustFundRemarks"),
...
export const getRemark = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get related data
    const fund = await ctx.db.get(remark.trustFundId);
...
/**
 * List all remarks for a trust fund
 */
export const listRemarksByFund = query({
  ...
  handler: async (ctx, args) => {
    let remarksQuery = ctx.db
      .query("trustFundRemarks")
      .withIndex("trustFundId", (q) =>
        q.eq("trustFundId", args.trustFundId)
      );
...
export const searchRemarks = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.trustFundId !== undefined) {
      ...
      remarks = await ctx.db.query("trustFundRemarks").collect();
    }
