...
export function BudgetStatusCell({ item }: BudgetStatusCellProps) {
    const [isUpdating, setIsUpdating] = useState(false);

    // Manage local state for immediate feedback
    const [localStatus, setLocalStatus] = useState<string>(item.status || "ongoing");
    const [isOpen, setIsOpen] = useState(false);

    const updateStatus = useMutation(api.budgetItems.updateStatus);
    ...
    const handleStatusChange = async (newStatus: "completed" | "ongoing" | "delayed") => {
        ...
        try {
            ...
            setLocalStatus(item.status || "ongoing"); // Revert on error
        } finally {
            setIsUpdating(false);
        }
    };
        ...
        <div className="flex items-center justify-center h-full" onClick={handleClick}>
            {isUpdating ? (
                ...
                >
                    <SelectTrigger
                        className={cn(
                            "h-7 w-[110px] text-xs border-0 bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:ring-0",
