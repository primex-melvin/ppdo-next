...
import { formatCurrency, formatPercentage } from "@/components/ppdo/shared/table/utils/formatters";
import { TwentyPercentDFTotals } from "@/components/ppdo/twenty-percent-df/types";
...
export function TwentyPercentDFResizableTotalsRow({
    columns,
    totals,
}: TwentyPercentDFResizableTotalsRowProps) {
        ...
        <tr className="sticky bottom-0 bg-zinc-50 dark:bg-zinc-800 font-bold z-20 shadow-[0_-1px_0_0_rgba(0,0,0,0.1)] dark:shadow-[0_-1px_0_0_rgba(255,255,255,0.1)]">
            ...
            {columns.map(column => {
                ...
                if (column.key === "totalBudgetAllocated") {
                    ...
                    cellContent = (
                        <span className={cn(
                            totals.utilizationRate >= 100 ? "text-green-600 dark:text-green-400" : ""
                        )}>
