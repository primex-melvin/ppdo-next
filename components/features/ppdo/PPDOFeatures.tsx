...
export default function PPDOFeatures() {
  ...
  const handleImageEnter = (src: string) => {
    setHoveredImage(src);
  };
  ...
  const sliderImages = [
    ...
    "/news-events/Government Project Monitoring and Compliance Review.png",
    "/news-events/Inter-Agency Planning and Coordination Workshop.png",
    "/news-events/Policy Implementation Monitoring and Evaluation Session.png",
    "/news-events/Research, Data Analysis, and Statistical Reporting.png",
  ...
  return (
    <>
      {/* LANDING PAGE BOXES */}
      <section className="px-4 py-5 bg-[#f8f8f8]">
          ...
          {features.map((feat, i) => {
            ...
            return (
              <motion.div
                ...
                "
              >
                <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-[#dff3e5] shadow-sm">
                  <Icon className="h-6 w-6 text-[#1b7a3c]" />
                </div>
                <p
        ...
        <div
          className="ppdo-carousel-container max-w-7xl mx-auto mt-10 overflow-hidden rounded-2xl border border-[#012130]/10 bg-[#f8f8f8]"
          onMouseLeave={handleImageLeave}
        >
          <div className={`ppdo-carousel-track gap-4 px-4 py-5 ${hoveredImage ? "ppdo-carousel-paused" : ""}`}>
            {[...sliderImages, ...sliderImages].map((src, idx) => (
              <div
                key={`${src}-${idx}`}
                className="h-24 w-36 flex-shrink-0 overflow-hidden rounded-xl bg-white shadow-sm"
              >
                <img
                  src={src}
                  alt="PPDO activity"
                  ...
                  loading="lazy"
                  onMouseEnter={() => handleImageEnter(src)}
                  onMouseLeave={handleImageLeave}
                />
              </div>
            ))}
      ...
      {hoveredImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 pointer-events-none">
          <div className="relative w-full max-w-3xl rounded-2xl bg-white p-4 shadow-2xl animate-fade-in-up">
            <div className="relative w-full overflow-hidden rounded-xl bg-gray-100">
              <img
                src={hoveredImage}
                alt="PPDO activity preview"
